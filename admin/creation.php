<?php 
  ...
  if(!empty($_POST)){
    ...
    if(empty($vignette || $galerieImg1 || $galerieImg2 || $galerieImg3 || $zip)){
      $galerieImg1Erreur = "Toutes les images ainsi que le fichier zip doivent être selectionés";
    }
    else{
      ...
      if($_FILES["vignette"]["size"] > 500000) {
        $vignetteErreur = "Le fichier ne doit pas depasser les 500KB";
        $isUploadSuccess = false;
      }
      ...
      if($_FILES["galerieImg1"]["size"] > 700000) {
        $galerieImg1Erreur = "Le fichier ne doit pas depasser les 700KB";
        $isUploadSuccess = false;
      }
      ...
      if($_FILES["galerieImg2"]["size"] > 700000) {
        $galerieImg2Erreur = "Le fichier ne doit pas depasser les 700KB";
        $isUploadSuccess = false;
      }
      ...
      if($_FILES["galerieImg3"]["size"] > 700000) {
        $galerieImg3Erreur = "Le fichier ne doit pas depasser les 700KB";
        $isUploadSuccess = false;
      }

      if($zipExtension != "zip") {
        $vignetteErreur = "Les fichiers autorises sont: .zip";
        $isUploadSuccess = false;
      }
      ...
      if($_FILES["zip"]["size"] > 5000000) {
        $vignetteErreur = "Le fichier ne doit pas depasser les 5000KB";
        $isUploadSuccess = false;
      }
      if($isUploadSuccess) {
    ...
    if($isSuccess && $isUploadSuccess) {
      $statement = $pdo->prepare("INSERT INTO projets (projetTitre, projetVignette, projetDescription, projetBut, projetResultat, projetTechnos, projetGalerieImg1, projetGalerieDesc1, projetGalerieImg2, projetGalerieDesc2, projetGalerieImg3, projetGalerieDesc3, projetTelechargement) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
      $statement->execute(array($titre, $vignette, $description, $but, $resultat, $technos, $galerieImg1, $galerieDesc1, $galerieImg2, $galerieDesc2, $galerieImg3, $galerieDesc3, $zip ));
  ...
  <body>
    ...
    <div class="container-fluid">
      <section id="creationProjet">
        <h2>Formulaire de Création de Projet</h2>
        <form class="form" action="creation.php" method="post" enctype="multipart/form-data">
